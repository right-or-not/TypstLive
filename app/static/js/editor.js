...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to close toolbox
    function closeToolbox() {
        mainWrapper.classList.add('toolbox-closed');
        // Show the floating open button with a slight delay to avoid visual clash
        setTimeout(() => {
            toolboxOpenBtn.style.display = 'flex';
        }, 300);
        // Force resize update for CodeMirror
        setTimeout(() => {
            if (window.EditorAPI && window.EditorAPI.refresh) {
                // 如果你有暴露 refresh 方法
            }
        }, 400);
    }
    ...
    function openToolbox() {
        toolboxOpenBtn.style.display = 'none'; // Hide open button immediately
        mainWrapper.classList.remove('toolbox-closed');
    }
    ...
    // Bind Events
    if (toolboxCloseBtn) {
        toolboxCloseBtn.addEventListener('click', closeToolbox);
    }
    
    if (toolboxOpenBtn) {
        toolboxOpenBtn.addEventListener('click', openToolbox);
    }
});
